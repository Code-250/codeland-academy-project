$(document).ready(function () {
  ...
  $(document).on("click", ".relatedHashtag .bar__item", function () {
    if (!$(this).hasClass("bar__item--active")) {
      ...
      getArticlesByTag(currentTag, max, orderBy, sortBy);
    }
  });
    ...
    if (data) {
      ...
      const emptyCard = `
        <a class='card' href=''>
          ...
          <h4 class='card__desc text-subtitle'></h4>
        </a>`;
  ...
  function getArticlesByTag(tag, length, orderBy, sortBy) {
    ...
    $.ajax({
      url: `/bin/relatedtags?tag=${tag}&length=${length}&orderBy=${orderBy}&sortBy=${sortBy}`,
      type: "GET",
