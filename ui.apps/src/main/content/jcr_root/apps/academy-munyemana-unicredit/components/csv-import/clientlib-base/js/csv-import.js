$(document).ready(function () {
    ...
    $("#btnSubmit").click(function (event) {
        ...
        if ($("#xls").val().length > 1) {
            ...
            $.ajax({
                ...
                url: "/bin/import_articles",
                data: fd,
                processData: false,
                contentType: false,
                cache: false,
                success: function (data) {
                    console.log(data, "this is your response")
                    json = JSON.parse(JSON.stringify(data));
                    CheckImportJob(json)
                },
                error: function (e) {
                    console.log(e, "this is the errror");
    ...
    document.querySelectorAll(".progress")[0].style.display="none"
});
;
var timeout;
...
function CheckImportJob(json){
    ...
    $.ajax({
        url: '/bin/import_article/status',
        type: 'get',
        dataType: 'json',
        contentType: 'application/json',
        success:function (data) {
            console.log("BBBBBBBB",data["statsPercentage"])
            if(!checkStarted){
            ...
            if(checkStarted && data["statsPercentage"] > 0 ){
                console.log("BBBBBBBB",data["statsPercentage"])
                ...
                sleep(1000).then(() => {
                    ...
                    $(".resultSkipped label").show();
                    $(".resultKo label").text("-Artcles with error: " + data["statsErrorRecords"]);
                    $(".resultKo label").show();
                    $(".loading").removeClass("loading--show").addClass("loading--hide");
