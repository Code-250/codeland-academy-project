package it.codeland.academy.core.jobs;
...
@Component(service = JobExecutor.class, immediate = true, property = {
        ...
        JobExecutor.PROPERTY_TOPICS + "=richard/job/topic"
})
public class JobExecute implements JobExecutor {
    ...
    public JobExecutionResult process(final Job job, JobExecutionContext context) {
        ...
        try {
            ...
            Resource resource = resolver.getResource("/content/dam/academy-munyemana-unicredit/articles.csv");
            if (resource == null) {
            ...
            logger.info("this is the size of you articles", statistics.totalArticle);
            int i = 0;
            ...
            while (rows.hasNext()) {
                ...
                if (row.length == 5) {
                    ...
                    if (pageManager
                        ...
                        logger.info("these are the article page" + articlePage);
                        if (articlePage != null) {
                            ...
                            if (articleContentNode != null) {
                                ...
                                logger.error("There is no \"{}\" content!", articleData.pageName + statistics);
                            }
                        } else {
                            logger.error("Cannot create {} page!", articleData.pageName + statistics);
                        }
                    } else {
                        statistics.skippedRecords++;
                        logger.info("Page \"{}\" already exists", articleData.pageName + statistics);
                    }
                } else {
            ...
            logger.info("Error occurs Hereeeeeeeeeeeeeeeeeeeeeeee: {}", e.getMessage());
            e.printStackTrace();
        }
